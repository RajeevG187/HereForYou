...
const MyProfile = () => {
  ...
  const {userData, setUserData, token, backendUrl, loadUserProfileData, updateAppointment} = useContext(AppContext)
    ...
    <div className='max-w-lg flex flex-col gap2 text-sm'>
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_2fr] gap-y-2.5 mt-3 text-neutral-700 '>
          ...
          {
            isEdit ?
              <select className ='max-w-20 bg-gray' value={userData.gender} onChange={(e) => setUserData(prev => ({ ...prev, gender: e.target.value }))}>
          ...
          {
            isEdit ?
              <input className ='max-w-28 bg-gray' type="date" value={userData.dob} onChange={(e) => setUserData(prev => ({ ...prev, dob: e.target.value }))} /> :
